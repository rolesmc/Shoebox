  ...
  try {
    ...
    while (!previewOut.includes("Local:")) {
      if (Date.now() - startedAt > PREVIEW_READY_TIMEOUT_MS) {
        fail(
          `vite preview did not emit "Local:" within ${PREVIEW_READY_TIMEOUT_MS / 1000}s`,
        );
    ...
    if (chromeAvailable) {
      ...
      info(
        `Headless Chrome polling (up to ${MAX_ATTEMPTS} attempts, ${BACKOFF_MS}ms backoff) …`,
      );
      let passingDom = null;
      let lastInnerLen = 0;
      for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
        ...
        if (/loaded outside DEV/i.test(stdout)) {
          fail(
            `prod bundle contains "loaded outside DEV" at offset ${stdout.search(/loaded outside DEV/i)} — Plan 06 regression`,
          );
          process.exit(1);
        }
        ...
        info(
          `  attempt ${attempt}: root inner length=${innerLen} (need > 200), retrying after ${BACKOFF_MS}ms`,
        );
        await sleep(BACKOFF_MS);
      }
      if (!passingDom) {
        fail(
          `root never rendered after ${MAX_ATTEMPTS} attempts (~${(MAX_ATTEMPTS * BACKOFF_MS) / 1000}s wall clock). Last inner length: ${lastInnerLen}. Possible mount regression or compile error.`,
        );
        process.exit(1);
      }
      ...
      if (/Uncaught Error/i.test(passingDom)) {
        fail(
          'DOM contains "Uncaught Error" — likely error-boundary fallback rendered',
        );
        process.exit(1);
      }
      ...
      if (!/class="[^"]*glass-card[^"]*"/.test(passingDom)) {
        fail(
          "DOM does NOT contain a `glass-card` class — design system CSS classes not mounted",
        );
        process.exit(1);
      }
      ...
      warn(
        "Using HTTP-only fallback — cannot detect runtime exceptions, only structural correctness.",
      );
      ...
      if (
        !/<div\s+id="root">\s*<\/div>|<div\s+id="root">\s*<\/div>/.test(
          indexHtml,
        ) &&
      ...
      for (const n of badNeedles) {
        if (bundle.includes(n)) {
          fail(
            `prod bundle contains forbidden marker "${n}" at offset ${bundle.indexOf(n)} in ${jsFiles[0]}`,
          );
          process.exit(1);
        }
      }
