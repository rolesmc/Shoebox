...
export default function StorageGauge({ state = 'empty' }) {
  const PRESETS = {
    ...
    'projected':   { used: 42,  projected: 75, label: '42 GB used → 75 GB after migration' },
    'over-quota':  { used: 95,  projected: 120, label: 'WARNING: 120 GB projected — exceeds 100 GB quota' },
  };
  ...
  return (
    <div className="glass-card" style={{ padding: '20px' }}>
      ...
      <div style={{ position: 'relative', height: '16px', borderRadius: '8px', background: 'rgba(255,255,255,0.06)', overflow: 'hidden' }}>
        <div style={{
        ...
        {projected > used && (
          <div style={{
            position: 'absolute', left: `${Math.min(used, 100)}%`, top: 0, bottom: 0,
            width: `${Math.min(projected - used, 100 - used)}%`,
            background: isOver ? 'var(--accent-purple)' : 'rgba(139, 92, 246, 0.4)',
          }} />
        )}
      </div>
      <div style={{ marginTop: '10px', color: isOver ? 'var(--accent-purple)' : 'var(--text-primary)', fontSize: '13px' }}>
        {label}
      </div>
      <div style={{ marginTop: '4px', color: 'var(--text-secondary)', fontSize: '11px', fontStyle: 'italic' }}>
