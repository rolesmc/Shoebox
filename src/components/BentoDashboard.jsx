...
function authCardStates(auth) {
  switch (auth) {
    case "both":
      return {
        source: { state: "connected", email: "student@school.edu" },
        dest: { state: "connected", email: "me@gmail.com" },
      };
    case "source-only":
      return {
        source: { state: "connected", email: "student@school.edu" },
        dest: { state: "disconnected" },
      };
    case "dest-only":
      return {
    ...
    case "expired":
      return {
        source: { state: "expired", email: "student@school.edu" },
        dest: { state: "expired", email: "me@gmail.com" },
      };
    default:
      return {
        source: { state: "disconnected" },
...
export default function BentoDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { getAllFiles, getAllFolders } =
          await import("../mocks/mockData.js");
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    (async () => {
      const { getAllFiles, getStressFiles } =
        await import("../mocks/mockData.js");
      if (cancelled) return;
      if (datasetSize <= 500) {
        setFiles(getAllFiles());
  ...
  return (
    <div className="bento-shell">
      <header className="bento-header">
        <h1
          style={{ margin: 0, fontSize: "24px", color: "var(--text-primary)" }}
        >
          UniVault
        </h1>
        <span style={{ color: "var(--text-secondary)", fontSize: "13px" }}>
          Phase 0 skeleton · {files.length.toLocaleString()} files · mock
          dataset
        </span>
      </header>
      ...
      {import.meta.env.DEV && (
        <DevPanel
          ...
          onDatasetSize={(n) => {
            setDatasetSize(n); /* Plan 05 hooks here */
          }}
      ...
      <div className="bento-grid">
        <div className="span-6">
          <AuthCard account="source" {...cards.source} />
        </div>
        <div className="span-6">
          <AuthCard account="dest" {...cards.dest} />
        </div>
