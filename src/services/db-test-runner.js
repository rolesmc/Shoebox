...
async function runTests() {
  try {
    ...
    try {
      ...
      log(
        "PASS: TokenStorage strictly threw error on 2KB payload boundary: " +
          e.message,
      );
    }
    ...
    if (
      ...
      log(
        "PASS: TokenStorage retrieved credentials perfectly match written values",
      );
    } else {
    ...
    const missingStores = expectedStores.filter(
      (name) => !objectStores.includes(name),
    );
    if (missingStores.length === 0) {
      log(
        "PASS: Four distinct object stores exist: " + objectStores.join(", "),
      );
    } else {
    ...
    if (
      ...
      log(
        "PASS: SelectionStore PubSub listener reactively fired and synced selection sets",
      );
    } else {
      log(
        `FAIL: SelectionStore listener events count mismatch: ${selectionEventCount}`,
      );
    }
    ...
    await QueueStore.updateTask("transfer_queue_001", {
      status: "copying",
      bytesCopied: 409600,
    });
    ...
    if (
      ...
      log(
        "PASS: QueueStore update transaction committed atomically and notified subscribers",
      );
    } else {
      log("FAIL: QueueStore invariant transaction check failed");
    }
    log(
      `INFO: Queue status write transaction done in ${(endTimestamp - startTimestamp).toFixed(2)}ms`,
    );
    unsubscribeQueue();
